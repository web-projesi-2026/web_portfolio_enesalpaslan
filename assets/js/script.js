...
function openTab(evt, tabName) {
  const tabsBody = evt.currentTarget.closest('.tabs-wrap').querySelector('.tabs-body');
  const tabsHead = evt.currentTarget.closest('.tabs-head');
  
  tabsBody.querySelectorAll('.tab-content').forEach(tc => tc.classList.remove('active'));
  tabsHead.querySelectorAll('.tab-btn').forEach(btn => btn.classList.remove('active'));
  
  document.getElementById(tabName).classList.add('active');
  evt.currentTarget.classList.add('active');
}

/* ═══ SLIDER (Müşteri Yorumları) ═══ */
...
if(tTrack && tDots) {
  ...
  const dotsList = tDots.querySelectorAll('.t-dot');

  function updateSlider() {
    const cWidth = tCards[0].offsetWidth;
    const style = window.getComputedStyle(tTrack);
    const gap = parseFloat(style.gap) || 0;
    const totalWidth = cWidth + gap;
    
    // Bounds check
    const maxVisible = window.innerWidth >= 1024 ? 3 : (window.innerWidth >= 768 ? 2 : 1);
    const maxIndex = Math.max(0, tCards.length - maxVisible);
    if(currentSlide > maxIndex) currentSlide = maxIndex;
    if(currentSlide < 0) currentSlide = 0;

    tTrack.style.transform = `translateX(-${currentSlide * totalWidth}px)`;
    
    dotsList.forEach((d, i) => d.classList.toggle('active', i === currentSlide));
    
    const pBtn = document.querySelector('.t-slider-btn.prev');
    const nBtn = document.querySelector('.t-slider-btn.next');
    if(pBtn) pBtn.disabled = currentSlide === 0;
    if(nBtn) nBtn.disabled = currentSlide >= maxIndex;
  }
